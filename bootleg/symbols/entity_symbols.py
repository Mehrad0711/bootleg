...
class EntitySymbols:
    ...
    def get_qid_cands(self, alias, max_cand_pad=False, allow_skip=False):
        """Get the QID candidates for an alias.

        Args:
            alias: alias
            max_cand_pad: whether to pad with '-1' or not if fewer than max_candidates candidates
            allow_skip: if the entity does not have any candidates return -1
             (sometimes happens when entity is extracted with neural model)
